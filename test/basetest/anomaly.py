...
from .utils import run_cmd_wait, run_cmd_wait_nofail, which, anomaly_binary_location, DEFAULT_EXTENSION_PATH
from .compat import STRING_TYPE


class Anomaly(object):
    """Manage a Anomaly instance

    A temporary folder is used as data store of anomaly.

    A anomaly client should not be used after being destroyed.
    """
    DEFAULT_ANOMALY = anomaly_binary_location()

    def __init__(self, anomaly=DEFAULT_ANOMALY):
        """Initialize a anomaly (client).
        The program runs in a temporary folder.

        :arg anomaly: Anomaly binary to use as client (defaults: anomaly in PATH)
        """
        self.anomaly = anomaly

        # Used to specify what command to launch (and to inject faketime)
        self._command = [self.anomaly]

        # Configuration of the isolated environment
        self._original_pwd = os.getcwd()
        self.datadir = tempfile.mkdtemp(prefix="anomaly_")
    ...
    def __repr__(self):
        txt = super(Anomaly, self).__repr__()
        return "{0} running from {1}>".format(txt[:-1], self.datadir)

    def __call__(self, *args, **kwargs):
        "aka t = Anomaly() ; t() which is now an alias to t.runSuccess()"
    ...
    def runSuccess(self, args="", input=None, merge_streams=False,
                   timeout=5):
        """Invoke anomaly with given arguments and fail if exit code != 0

        Use runError if you want exit_code to be tested automatically and
        *not* fail if program finishes abnormally.

        If you wish to pass instructions to anomaly such as confirmations or other
        ...
        timeout = number of seconds the test will wait for every anomaly call.
